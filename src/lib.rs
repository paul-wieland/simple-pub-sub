...
pub async fn run() -> Result<(), String>{
    ...
    let create_topic_use_case =
        Arc::new(
            CreateTopicUseCase::new(
                Box::new(TopicPersistenceAdapter::new().await.map_err(|_| { "Error" })?)));

    let get_topics_use_case =
        Arc::new(
    ...
    let create_message_use_case = Arc::new(
        CreateMessageUseCase::new(
            Box::new(MessagePersistenceAdapter::new().await.map_err(|_| { "Error" })?),
            Box::new(SubscriptionPersistenceAdapter::new().await.map_err(|_| { "Error" })?),
